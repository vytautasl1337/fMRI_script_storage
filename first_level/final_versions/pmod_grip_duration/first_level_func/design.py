...
def design_mat(fmri_task,beh_table,events_dir,physio_dir,comb,subject_id,task_label,t_r,
    ...
    for id_, img in enumerate(fmri_task):
        ...
        pmod_duration = subject_beh['GripDuration']
        pmod_duration = pmod_duration.fillna(0.2)
        pmod_duration = pmod_duration.reset_index(drop=True)
        pmod_onset_ = subject_beh['RT_init']
        pmod_onset_ = pmod_onset_.fillna(0)
        pmod_onset_reindexed = pmod_onset_.reset_index(drop=True)
        events_table_onset_reindexed = events_table['onset'].reset_index(drop=True)
        pmod_onset = pmod_onset_reindexed+events_table_onset_reindexed
        
        slopes=pd.DataFrame()
        slopes=pd.DataFrame({'slopes': subject_beh['Slope'], 'grips': go_conditions,
                             'duration': pmod_duration.tolist(), 'onset':pmod_onset.tolist()})
