...
def design_mat(fmri_task,beh_table,events_dir,physio_dir,comb,subject_id,task_label,t_r,
    ...
    for id_, img in enumerate(fmri_task):
        ...
        filtered_slopes = slopes[slopes['grips'] != 'NoGo']
        nogo_slopes = slopes[slopes['grips'] == 'NoGo']
        left_slopes = filtered_slopes[filtered_slopes['grips']=='LeftGo']
        right_slopes = filtered_slopes[filtered_slopes['grips']=='RightGo']
        left_mean = left_slopes['slopes'].mean()
        right_mean = left_slopes['slopes'].mean()
        left_slopes['slopes'] = left_slopes['slopes'] - left_mean
        right_slopes['slopes'] = right_slopes['slopes'] - right_mean
        hand_slopes = pd.concat([left_slopes,right_slopes],axis=0)
        
        slopes_df = pd.DataFrame({'trial_type': hand_slopes['grips'], 
                                         'onset': hand_slopes['onset'],
                                        'duration': hand_slopes['duration'],
                                        'modulation':hand_slopes['slopes']})
        frame_times = numpy.arange(n_scans) * t_r
        pmod_matrix = make_first_level_design_matrix(
                frame_times,
                slopes_df,
                hrf_model='spm',
                high_pass=0.0078,
        ...
        design_matrix['RightGo_pmod'] = float('nan')
        design_matrix['RightGo_pmod'] = pmod_matrix['RightGo']
        design_matrix['LeftGo_pmod'] = float('nan')
        design_matrix['LeftGo_pmod'] = pmod_matrix['LeftGo']
        ...
        order = ['LeftGo','LeftGo_pmod','RightGo','RightGo_pmod','NoGo','Happy','Sad','Neutral', 
                   'HighReward', 'LowReward', 'Win', 'Loss', 'Error']
