...
def design(contrast1,contrast,save_folder,exclude_list, 
    ...
    for b in range(beh_ref.shape[0]):
        ...
        sub_data = beh_mri[((beh_mri.Run == 1) | (beh_mri.Run == 2)) & 
                           (beh_mri.id == behav_code) & (beh_mri.Correct == 1) & (beh_mri.GripResponse != 0)]
        ...
        elif cond == 'right go high':
            beh12 = sub_data[(sub_data['RewardPromise'] == 1) & (sub_data['GripResponse'] == 1)][behavioral].mean()
        elif cond == 'right go low':
            beh12 = sub_data[(sub_data['RewardPromise'] == -1) & (sub_data['GripResponse'] == 1)][behavioral].mean()
        elif cond == 'left go high':
            beh12 = sub_data[(sub_data['RewardPromise'] == 1) & (sub_data['GripResponse'] == -1)][behavioral].mean()
        elif cond == 'left go low':
            beh12 = sub_data[(sub_data['RewardPromise'] == -1) & (sub_data['GripResponse'] == -1)][behavioral].mean()
        ...
        dictio = {'SubNr':sub_code,'Code':behav_code,'BEH1':beh12, 'age':age_mean, 'ehi':ehi_mean, 'gender':gender_mean}
    ...
    missing_data = df_clean['SubNr'].loc[pd.isna(df_clean['BEH1'])].unique()
    ...
    for sub in range(len(participants_list)):
        ...
        if subject_id not in exclude_list:
            stat_map1 = os.path.join(working_dir,
                                'derivatives/task_output/FirstLevel/FirstLevel_1and2_run_03hz/{}/first_level_maps/{}_contrast-{}_stat-z_statmap.nii.gz'.format(subject_id,subject_id,contrast1))
            # stat_map2 = os.path.join(working_dir,
            #                     'derivatives/task_output/FirstLevel_pmods_nogender_fixed/{}/first_level_maps/{}_contrast-{}_stat-z_statmap.nii.gz'.format(subject_id,subject_id,contrast2))
    ...
    # beh_z_beh1 = np.concatenate([beh_z['BEH1'], np.zeros_like(beh_z['BEH1'])])
    # beh_z_beh2 = np.concatenate([np.zeros_like(beh_z['BEH2']),beh_z['BEH2']])
    ...
    beh_1 = np.array(beh_z['BEH1']).reshape(-1, 1)
    ...
    np.hstack((condition_effect1[:, np.newaxis],
               beh_1,age_,gender_)),
    columns=[contrast1,behavioral,'age','gender'],
    )
