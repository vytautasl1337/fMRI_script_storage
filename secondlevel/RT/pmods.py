...
def pmod_func(index,session,p_value,run):
    ...
    for folder in ['FirstLevel_pmod_normal_RT']:#,'FirstLevel_pmod_gripdur']:

        if folder == 'FirstLevel_pmod_normal_RT':
            output_folder1 = os.path.join(working_dir,
                'derivatives/task_output/SecondLevel/pmod_normal_noslope_RT/{}/{}/{}'.format(p_value,run,contrast))
            output_folder_non1 = os.path.join(working_dir,
                'derivatives/task_output/SecondLevel/pmod_normal_noslope_nonparam_RT/{}/{}/{}'.format(p_value,run,contrast))
        elif folder == 'FirstLevel_pmod_gripdur':
            output_folder1 = os.path.join(working_dir,
                'derivatives/task_output/SecondLevel/pmod_grip_duration_noslope/{}/{}/{}'.format(p_value,run,contrast))
            output_folder_non1 = os.path.join(working_dir,
                'derivatives/task_output/SecondLevel/pmod_grip_duration_noslope_nonparam/{}/{}/{}'.format(p_value,run,contrast))
        ...
        if folder == 'FirstLevel_pmod_normal_RT':
            output_folder2 = os.path.join(working_dir,
                'derivatives/task_output/SecondLevel/pmod_normal_withslope_RT/{}/{}/{}'.format(p_value,run,contrast))
            output_folder_non2 = os.path.join(working_dir,
                'derivatives/task_output/SecondLevel/pmod_normal_withslope_nonparam_RT/{}/{}/{}'.format(p_value,run,contrast))
        elif folder == 'FirstLevel_pmod_gripdur':
            output_folder2 = os.path.join(working_dir,
                'derivatives/task_output/SecondLevel/pmod_grip_duration_withslope/{}/{}/{}'.format(p_value,run,contrast))
            output_folder_non2 = os.path.join(working_dir,
                'derivatives/task_output/SecondLevel/pmod_grip_duration_withslope_nonparam/{}/{}/{}'.format(p_value,run,contrast))
        ...
        for b in range(beh_ref.shape[0]):
            ...
            if contrast in ['goPmod']:
                slope = sub_data['RT_init'].mean()
                
            elif contrast in ['rightgoPmod']:
                slope = sub_data[(sub_data['GripResponse'] == 1)]['RT_init'].mean()
                
            elif contrast in ['leftgoPmod']:
                slope = sub_data[(sub_data['GripResponse'] == -1)]['RT_init'].mean()
                
            elif contrast in ['leftrightPmod']:
                slope_left = sub_data[(sub_data['GripResponse'] == -1)]['RT_init'].mean()
                slope_right = sub_data[(sub_data['GripResponse'] == 1)]['RT_init'].mean()
                slope = slope_left-slope_right
                
            elif contrast in ['rightleftPmod']:
                slope_left = sub_data[(sub_data['GripResponse'] == -1)]['RT_init'].mean()
                slope_right = sub_data[(sub_data['GripResponse'] == 1)]['RT_init'].mean()
                slope = slope_right - slope_left
                
            elif contrast in ['gohighPmod']:
                slope = sub_data[(sub_data['RewardPromise'] == 1)]['RT_init'].mean()
                
            elif contrast in ['golowPmod']:
                slope = sub_data[(sub_data['RewardPromise'] == -1)]['RT_init'].mean()
                
            elif contrast in ['gohighrightPmod']:
                slope = sub_data[(sub_data['RewardPromise'] == 1)&(sub_data['GripResponse']==1)]['RT_init'].mean()
                
            elif contrast in ['gohighleftPmod']:
                slope = sub_data[(sub_data['RewardPromise'] == 1)&(sub_data['GripResponse']==-1)]['RT_init'].mean()
                
            elif contrast in ['golowrightPmod']:
                slope = sub_data[(sub_data['RewardPromise'] == -1)&(sub_data['GripResponse']==1)]['RT_init'].mean()
                
            elif contrast in ['golowleftPmod']:
                slope = sub_data[(sub_data['RewardPromise'] == -1)&(sub_data['GripResponse']==-1)]['RT_init'].mean()
                
            elif contrast in ['highlowPmod']:
                slope_high = sub_data[(sub_data['RewardPromise'] == 1)]['RT_init'].mean()
                slope_low = sub_data[(sub_data['RewardPromise'] == -1)]['RT_init'].mean()
                slope = slope_high - slope_low
                
            elif contrast in ['lowhighPmod']:
                slope_high = sub_data[(sub_data['RewardPromise'] == 1)]['RT_init'].mean()
                slope_low = sub_data[(sub_data['RewardPromise'] == -1)]['RT_init'].mean()
                slope = slope_low - slope_high
                
            elif contrast in ['righthighlowPmod']:
                slope_high = sub_data[(sub_data['RewardPromise'] == 1)&(sub_data['GripResponse']==1)]['RT_init'].mean()
                slope_low = sub_data[(sub_data['RewardPromise'] == -1)&(sub_data['GripResponse']==1)]['RT_init'].mean()
                slope = slope_high - slope_low
                
            elif contrast in ['lefthighlowPmod']:
                slope_high = sub_data[(sub_data['RewardPromise'] == 1)&(sub_data['GripResponse']==-1)]['RT_init'].mean()
                slope_low = sub_data[(sub_data['RewardPromise'] == -1)&(sub_data['GripResponse']==-1)]['RT_init'].mean()
                slope = slope_high - slope_low
        ...
        condition_effect = np.hstack(([1] * n_subjects))
        slope_ = np.array(beh_z['RT_init']).reshape(-1, 1)


        design_matrix = pd.DataFrame(
            np.hstack((condition_effect[:, np.newaxis],slope_)), columns=["intercept","RT_init"]
        )
        ...
        # conjuction
        map_ = second_level_model.compute_contrast('RT_init')
