...
output = os.path.join(working_dir,
                      'derivatives/task_output/SecondLevel_tfce/tfce_slope_cov/{}'.format(run))
...
for b in range(beh_ref.shape[0]):
    ...
    if contrast in ['sadneutral']:
        beh1 = sub_data[sub_data['FaceEmotion'] == -1]['Slope'].mean()
        beh2 = sub_data[sub_data['FaceEmotion'] == 0]['Slope'].mean()
        beh12 = beh1-beh2
    if contrast in ['sadhappy']:
        beh1 = sub_data[sub_data['FaceEmotion'] == -1]['Slope'].mean()
        beh2 = sub_data[sub_data['FaceEmotion'] == 1]['Slope'].mean()
        beh12 = beh1-beh2
    ...
    if contrast in ['winloss']:
        ...
        beh12 = beh1-beh2
    if contrast in ['highlow','highlowPmod']:
...
ols_outputs = permuted_ols(
    beh_vars,  # this is equivalent to the design matrix, in array form
    gm_data,
...
neg_log_pvals_tfce_unmasked = grey_matter_masker.inverse_transform(
    ols_outputs["logp_max_tfce"][0, :]  # select first regressor
)
neg_log_pvals_tfce_unmasked.to_filename(os.path.join(cont_output,'{}_neg_log_pvals_tfce.nii.gz'.format(contrast)))
