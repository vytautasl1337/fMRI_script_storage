...
for id_, row in beh_ref.iterrows():
    ...
    for run in range(3):
        for rew in range(2):
            ...
            behavioral_data.append({'Participant ID': id_+1, 'Run Number': run+1, 'Condition': condition, 'Averaged Slope': slope})
...
masker = NiftiMasker(mask_img=mask, standardize=True, smoothing_fwhm=8)
fmri_data = masker.fit_transform(nifti_files)

# Create design matrix
design_matrix = np.column_stack((behavioral_df['Run Number'], behavioral_df['Condition'], fmri_data))
...
model = Lmer('Averaged Slope ~ Run Number + Condition + (1|Participant ID)', data=behavioral_df)
results = model.fit()
